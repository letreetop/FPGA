...
entity nn_rgb is -- defines a module called nn_rgb and port in itself specifices the inputs and outputs of nn_rgb
  port (clk       : in  std_logic;                      -- input clock 74.25 MHz, video 720p
        reset_n   : in  std_logic;                      -- reset (invoked during configuration)
        enable_in : in  std_logic_vector(2 downto 0);   -- three slide switches (3bit signal)
        -- video in
        vs_in     : in  std_logic;                      -- vertical sync
        hs_in     : in  std_logic;                      -- horizontal sync
        de_in     : in  std_logic;                      -- data enable is '1' for valid pixel
        r_in      : in  std_logic_vector(7 downto 0);   -- red component of pixel (8bit signal)
...
architecture behave of nn_rgb is -- begins archeticture for behave as nn_rgb 

    -- input FFs
    signal reset                   : std_logic;
    signal enable                  : std_logic_vector(2 downto 0);
    signal vs_0, hs_0, de_0        : std_logic;
    signal r_0, g_0, b_0           : integer; -- input neurons?
...
control: entity work.control
    generic map (delay => 9) 
    port map (  clk      => clk, -- not assignment, but port mapping as in "soldering" pins together =>
                reset    => reset,
                vs_in    => vs_0,
                hs_in    => hs_0,
                de_in    => de_0,
...
begin   
    ...
    hs_0  <= hs_in;
    de_0  <= de_in;
    r_0   <= to_integer(unsigned(r_in)); -- this is variable assignment <=
